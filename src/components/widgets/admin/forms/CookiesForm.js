...
const CookiesForm = ({ newCake, setNewCake, newSize, setNewSize, handleAddSize, handleRemoveSize, cakes = [], editingCake }) => {
  // Ensure Cookies category is always selected
  useEffect(() => {
    if (!newCake.categories.includes('Cookies')) {
      setNewCake(prev => ({
        ...prev,
          ...
          {newCake.sizes.map((size, index) => (
            <div key={index} className="cakemanagement-size-item">
              <div className="cakemanagement-size-info">
                <span className="cakemanagement-size-name">{size.size} cookies</span>
